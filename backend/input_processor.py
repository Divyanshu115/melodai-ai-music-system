...
class InputProcessor:
    """
    Gemini-based Input Processor with safe JSON parsing.
    Works with both Streamlit (st.secrets) and CLI (env vars).
    """

    def __init__(self):
        # --- Get API key (Streamlit OR normal env) ---
        api_key = None
        try:
...
def _fallback(self, text: str) -> Dict[str, Any]:
    ...
    # -------- TEMPO --------
    tempo = "slow" if energy <= 3 else "fast" if energy >= 7 else "medium"
    ...
    # -------- FINAL PROMPT --------
    prompt = f"{style} {mood} music with {' '.join(instruments)}, {tempo} tempo, high quality, immersive sound"
